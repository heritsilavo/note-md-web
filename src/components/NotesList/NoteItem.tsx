...
export default function NoteItem({ note }: NoteItemProps) {
  ...
  return <div onClick={onClickNote} className="bg-white rounded-lg p-6 shadow flex flex-col justify-between cursor-pointer hover:shadow-lg active:shadow transition-shadow duration-200">
    ...
    <div onClick={(e) => { e.stopPropagation();router.push(`/note-editor/?action=edit_note&id=${note.supabase_id}`) }} className="flex justify-end gap-2 mt-2">
      <button className="text-blue-700 text-xs cursor-pointer hover:underline p-2">Modifier</button>
    </div>
  </div>
}